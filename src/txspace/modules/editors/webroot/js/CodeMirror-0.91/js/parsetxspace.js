var txSpaceParser = Editor.Parser = (function() {
	...
	var txspace_functions = ['get_dobj', 'get_dobj_str', 'has_dobj', 'has_dobj_str', 'get_pobj', 'get_pobj_str',
								'has_pobj', 'has_pobj_str', 'get_obj', 'new_obj', 'sudo', 'count_names'];
	
	var txspace_objects = ['system', 'caller', 'here', 'this'];
	
	var txspace_methods = ['get_id', 'get_name', 'set_name', 'has_child', 'has_parent', 'get_children', 'get_parent',
	...
	function configure(conf) {
		...
		//builtinFunctions = wordRegexp(functions.concat(txspace_functions).concat(txspace_methods));
		builtinFunctions = wordRegexp(functions.concat(txspace_functions));
		builtinExceptions = wordRegexp(exceptions);
		builtinObjects = wordRegexp(objects.concat(txspace_objects));
		txSpaceMethods = wordRegexp(txspace_methods);
	}
		...
		function normal(source, setState) {
			...
			if (identifierStarters.test(ch)) {
				...
				if (wordOperators.test(word)) {
					...
					while (source.peek() == '.') {
						source.next();
						if(txSpaceMethods.test(source.peek())) {
